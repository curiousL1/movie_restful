...
随着后续路由的增加，需要重构一个这个文件，仿照 `beego`的工程目录，我们也分别创建对应的 `response`
和 `routes`,改造一下上面的代码

* 控制器

创建 `response` 文件夹和对应的文件 `movies.go`  
...
import (
	...
	"github.com/coderminer/restful/response"
	"github.com/gorilla/mux"
)
...
func init() {
	register("GET", "/movies", response.AllMovies, nil)
	register("GET", "/movies/{id}", response.FindMovie, nil)
	register("POST", "/movies", response.CreateMovie, nil)
	register("PUT", "/movies", response.UpdateMovie, nil)
	register("DELETE", "/movies", response.DeleteMovie, nil)
}
...
func CreateMovie(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&movie); err != nil {
		responseWithJsonObject(w, http.StatusBadRequest, "Invalid request payload")
		return
	}
	movie.Id = bson.NewObjectId()
	if err := dao.InsertMovie(movie); err != nil {
		responseWithJsonObject(w, http.StatusInternalServerError, err.Error())
		return
	}
	responseWithJsonObject(w, http.StatusCreated, movie)
}
```
...
func AllMovies(w http.ResponseWriter, r *http.Request) {
	...
	responseWithJsonObject(w, http.StatusOK, movies)
...
func FindMovie(w http.ResponseWriter, r *http.Request) {
	...
	responseWithJsonObject(w, http.StatusOK, result)
}
```
...
func UpdateMovie(w http.ResponseWriter, r *http.Request) {
	...
	responseWithJsonObject(w, http.StatusOK, map[string]string{"result": "success"})
}
```
